    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await axios.get(`http://localhost:3000/series/${id}`); // Substitua com a sua URL e endpoint correto
    ...
    return (
        <Grid container justifyContent={"center"} padding={2} style={{ backgroundColor: "#efcfad", backgroundClip: "content-box", borderRadius: 30 }}>
            <Grid item xs={12} md={8} padding={2}>
                <div style={{ fontSize: 30, fontWeight: "bold", marginBottom: 10 }}>{serie.titulo?.nome}</div>
                <div style={{ fontSize: 20, fontWeight: "bold", marginBottom: 10 }}>{serie.quant_temporada} Temporadas, {serie.quant_episodio} Episódios</div>
                <div style={{ marginBottom: 20 }}>{serie.titulo?.sinopse}</div>
                ...
                <div style={{ display: "flex", justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                        ...
                        <div style={{ marginLeft: 20 }}>
                            {
                                serie.titulo?.where_view ?
                                    <img style={{ backgroundColor: "white", borderRadius: 10 }} width={100} src={ENTERPRISES[serie.titulo?.where_view.toLowerCase()].logo} alt="Onde Assistir" /> : false
                            }
                ...
                <div style={{ marginTop: 50 }}>
                    <span style={{ height: "100%", width: 10, backgroundColor: "orange", borderRadius: 5 }}>&nbsp;&nbsp;</span>
                    <span style={{ fontSize: 20, fontWeight: "bold" }}>&nbsp;Episódios</span>

                    {serie.temporadas &&
                        serie.temporadas.map((temporada) => (
                            <div key={temporada.id}>
                                <div style={{ fontSize: 18, fontWeight: "bold", marginTop: 10 }}>Temporada: {temporada.nome}</div>
                                {temporada.episodios &&
                                    temporada.episodios.map((episodio) => (
                                        <Episode key={episodio.id} episode={episodio} />
                                    ))}
                            </div>
                        ))}
                </div>
            ...
            <Grid item xs={12} md={4} style={{ textAlign: "center" }} padding={2}>
                <div>
                    <img height={"100%"} style={{ borderRadius: 10 }} src={serie.titulo?.poster} alt={`Poster de ${serie.titulo?.nome}`} />
